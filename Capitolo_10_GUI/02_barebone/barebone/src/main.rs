...
/**
    ...
    Questa demo mostra come utilizzare NWG senza il boilerplate del tratto NativeUi.
    Si noti che questo modo di fare le cose è molto meno estensibile 
    e non può utilizzare Windows derivato nativo.

    Vedi `basic` per la versione NativeUi e `basic_d` per la versione derivata
...
fn main() {
    ...
    nwg::Window::builder()
        .size((500, 115)) //AUMENTATO DA 300---> 500
        .position((300, 300))
        .title("STATIC UI - Basic example SOLO NWG IL NATIVEUI")
        .build(&mut window)
        .unwrap();
